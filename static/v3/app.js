...
function renderRestaurants() {
  ...
  grid.innerHTML = list.map((r, index) => {
    ...
    return `<article class="restaurant-card"><div class="card-image"><img src="/v3-assets/${info.image}" alt="Illustration of a plated meal" width="600" height="390" loading="lazy"><span class="card-tag">${escapeHTML(info.mood)}</span><span class="card-number">0${index + 1}</span></div><div class="card-main"><div class="card-topline"><span>${escapeHTML(info.type)}</span><span>✳ A GOOD FIND</span></div><h3>${escapeHTML(r.name)}</h3><p>${escapeHTML(info.description)}</p><div class="card-status"><span>${status} · UP TO ${r.capacity} GUESTS</span><b class="${available ? "" : "unavailable"}">${remaining}</b></div><button class="card-action" type="button" data-book="${r.id}" ${available ? "" : "disabled"}>${action} <span aria-hidden="true">↗</span></button></div></article>`;
  }).join("");
}
